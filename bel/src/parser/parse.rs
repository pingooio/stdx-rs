use std::{iter::Enumerate, num::ParseIntError, str::Chars};
...
pub fn parse_string(s: &str) -> Result<String, ParseSequenceError> {
    ...
    match chars.next() {
        ...
        Some((_, c)) if c == '"' => parse_quoted_string(s, &mut chars, res, c),
        _ => Err(ParseSequenceError::MissingOpeningQuote),
    }
}

fn parse_raw_string(chars: &mut Enumerate<Chars>, mut res: String) -> Result<String, ParseSequenceError> {
    // let mut in_single_quotes = false;
    let mut in_double_quotes = false;

    while let Some((_, c)) = chars.next() {
        // let in_quotes = in_single_quotes || in_double_quotes;
        let in_quotes = in_double_quotes;
                ...
                Some((idx, c2)) => {
                    ...
                    let value = match c2 {
                        ...
                        'x' | 'u' | 'U' => {
                            ...
                            parse_unicode_hex(length, &mut chars).map_err(|x| ParseSequenceError::InvalidUnicode {
                                source: x.clone(),
                                index: idx,
                                string: String::from(s),
                            })?
    ...
    I: Iterator<Item = (usize, char)>,
{
    ...
    u32::from_str_radix(&unicode_seq, 16)
        ...
        .and_then(|u| {
            char::from_u32(u).ok_or(ParseUnicodeError::Unicode {
                value: u,
            })
        })
}
...
mod tests {
    ...
    fn double_quotes_interprets_escapes() {
        let tests: Vec<(&str, Result<String, ParseSequenceError>)> = vec![
            ...
            ("\"Hello \\U0001f431\"", Ok(String::from("Hello 🐱"))),
            ("\"Hello \\040\"", Ok(String::from("Hello  "))),
            ("Missing closing quote\"", Err(ParseSequenceError::MissingOpeningQuote)),
