...
pub trait CELVisitor<'input>: ParseTreeVisitor<'input, CELParserContextType> {
     ...
     * @param ctx the parse tree
     */
    fn visit_Float(&mut self, ctx: &DoubleContext<'input>) {
        self.visit_children(ctx)
    }
...
pub trait CELVisitorCompat<'input>: ParseTreeVisitorCompat<'input, Node = CELParserContextType> {
     ...
     * @param ctx the parse tree
     */
    fn visit_Float(&mut self, ctx: &DoubleContext<'input>) -> Self::Return {
        self.visit_children(ctx)
    }
    ...
    T: CELVisitorCompat<'input>,
{
    ...
    fn visit_Float(&mut self, ctx: &DoubleContext<'input>) {
        let result = <Self as CELVisitorCompat>::visit_Float(self, ctx);
        *<Self as ParseTreeVisitorCompat>::temp_result(self) = result;
    }
