...
use crate::{
    ...
    parser::{
        generated,
        generated::{
            BoolFalseContext, BoolTrueContext, BytesContext, CalcContext, CalcContextAttrs, ConditionalAndContext,
            ConditionalOrContext, ConstantLiteralContext, ConstantLiteralContextAttrs, CreateListContext,
            CreateMessageContext, CreateStructContext, DoubleContext, ExprContext, FieldInitializerListContext,
            GlobalCallContext, IdentContext, IndexContext, IndexContextAttrs, IntContext, ListInitContextAll,
            LogicalNotContext, LogicalNotContextAttrs, MapInitializerListContextAll, MemberCallContext,
            MemberCallContextAttrs, MemberExprContext, MemberExprContextAttrs, NegateContext, NegateContextAttrs,
            NestedContext, NullContext, OptFieldContextAttrs, PrimaryExprContext, PrimaryExprContextAttrs,
            RelationContext, RelationContextAttrs, SelectContext, SelectContextAttrs, StartContext, StartContextAttrs,
...
impl Display for ParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "ERROR: <input>:{}:{}: {}", self.pos.0, self.pos.1, self.msg)?;
...
impl Parser {
    ...
    pub fn parse(mut self, source: &str) -> Result<IdedExpr, ParseErrors> {
        ...
        let mut lexer = generated::CELLexer::new(stream);
        ...
        let mut prsr = generated::CELParser::new(CommonTokenStream::new(lexer));
    ...
    fn field_initializer_list(&mut self, ctx: &FieldInitializerListContext<'_>) -> Vec<IdedEntryExpr> {
...
impl<'a, T: Recognizer<'a>> ErrorListener<'a, T> for ParserErrorListener {
    fn syntax_error(
        ...
        match offending_symbol {
            Some(offending_symbol) if offending_symbol.get_token_type() == generated::cellexer::WHITESPACE => {}
...
impl ParseTreeVisitorCompat<'_> for Parser {
    type Node = generated::CELParserContextType;
...
impl generated::CELVisitorCompat<'_> for Parser {
    ...
    fn visit_expr(&mut self, ctx: &ExprContext<'_>) -> Self::Return {
        match &ctx.op {
            None => match &ctx.e {
                None => self.report_error::<ParseError, _>(ctx.start().deref(), None, "No `ConditionalOrContextAll`!"),
            ...
            Some(op) => {
                if let (Some(e), Some(e1), Some(e2)) = (&ctx.e, &ctx.e1, &ctx.e2) {
                    ...
                    self.report_error::<ParseError, _>(
                        ctx.start().deref(),
                        None,
                        format!("Incomplete `ExprContext` for `{}` expression!", operators::CONDITIONAL),
                    )
                }
            }
        }
    ...
    fn visit_conditionalOr(&mut self, ctx: &ConditionalOrContext<'_>) -> Self::Return {
        let result = match &ctx.e {
            None => {
                self.report_error::<ParseError, _>(ctx.start().deref(), None, "No `ConditionalAndContextAll`!");
                IdedExpr::default()
            }
            Some(e) => <Self as ParseTreeVisitorCompat>::visit(self, e.as_ref()),
        };
    ...
    fn visit_conditionalAnd(&mut self, ctx: &ConditionalAndContext<'_>) -> Self::Return {
        let result = match &ctx.e {
            None => self.report_error::<ParseError, _>(ctx.start().deref(), None, "No `RelationContextAll`!"),
            Some(e) => <Self as ParseTreeVisitorCompat>::visit(self, e.as_ref()),
        };
        if ctx.ops.is_empty() {
            result
    ...
    fn visit_relation(&mut self, ctx: &RelationContext<'_>) -> Self::Return {
        if ctx.op.is_none() {
            match ctx.calc() {
                None => self.report_error::<ParseError, _>(ctx.start().deref(), None, "No `CalcContextAll`!"),
    ...
    fn visit_calc(&mut self, ctx: &CalcContext<'_>) -> Self::Return {
        match &ctx.op {
            None => match &ctx.unary() {
                None => self.report_error::<ParseError, _>(ctx.start().deref(), None, "No `UnaryContextAll`!"),
    ...
    fn visit_Select(&mut self, ctx: &SelectContext<'_>) -> Self::Return {
        if let (Some(member), Some(id), Some(op)) = (&ctx.member(), &ctx.id, &ctx.op) {
            ...
            if let Some(_opt) = &ctx.opt {
                return self.report_error::<ParseError, _>(op.as_ref(), None, "unsupported syntax '.?'");
            }
    ...
    fn visit_Ident(&mut self, ctx: &IdentContext<'_>) -> Self::Return {
        match &ctx.id {
            ...
            Some(id) => {
                ...
                self.helper.next_expr(id.deref(), Expr::Ident(ident.to_string()))
            }
        }
    }
    ...
    fn visit_ConstantLiteral(&mut self, ctx: &ConstantLiteralContext<'_>) -> Self::Return {
        <Self as ParseTreeVisitorCompat>::visit(self, ctx.literal().as_deref().expect("Has to have literal!"))
    }
    ...
    fn visit_Int(&mut self, ctx: &IntContext<'_>) -> Self::Return {
        ...
        self.helper.next_expr(token, Expr::Literal(CelVal::Int(val)))
    }
    ...
    fn visit_Uint(&mut self, ctx: &UintContext<'_>) -> Self::Return {
        ...
        self.helper.next_expr(token, Expr::Literal(CelVal::UInt(val)))
    }

    fn visit_Float(&mut self, ctx: &DoubleContext<'_>) -> Self::Return {
        let string = ctx.get_text();
        let token = ctx.tok.as_ref().expect("Has to have float!");
        match string.parse::<f64>() {
            Ok(d) if d.is_finite() => self.helper.next_expr(token, Expr::Literal(CelVal::Float(d))),
            Err(e) => self.report_error(token, Some(e), "invalid float literal"),
            _ => self.report_error(token, None::<ParseError>, "invalid float literal"),
        }
    }
    ...
    fn visit_String(&mut self, ctx: &StringContext<'_>) -> Self::Return {
        ...
        match parse::parse_string(&ctx.get_text()) {
            Ok(string) => self.helper.next_expr(token, Expr::Literal(CelVal::String(string))),
            Err(e) => self.report_error::<ParseError, _>(token, None, format!("invalid string literal: {e:?}")),
        }
    }
    ...
    fn visit_Bytes(&mut self, ctx: &BytesContext<'_>) -> Self::Return {
        ...
        match parse::parse_bytes(&string[2..string.len() - 1]) {
            Ok(bytes) => self.helper.next_expr(token, Expr::Literal(CelVal::Bytes(bytes))),
            Err(e) => {
    ...
    fn visit_Null(&mut self, ctx: &NullContext<'_>) -> Self::Return {
        self.helper
            .next_expr(ctx.tok.as_deref().expect("Has to be `null`!"), Expr::Literal(CelVal::Null))
    }
}
...
impl ParserHelper {
    fn next_id(&mut self, token: &CommonToken) -> u64 {
        let id = self.next_id;
        self.source_info.add_offset(id, token.start as u32, token.stop as u32);
        self.next_id += 1;
        id
    }
...
mod tests {
    ...
    use crate::{
        IdedExpr,
        common::ast::{ComprehensionExpr, EntryExpr, Expr},
    };
    ...
    -4^#3:*expr.Constant_Int64Value#
            ...
            TestInfo {
                ...
                p: r#"_-_(
    4^#1:*expr.Constant_Int64Value#,
    -4.1^#3:*expr.Constant_FloatValue#
)^#2:*expr.Expr_CallExpr#"#,
                e: "",
            },
            TestInfo {
            ...
            TestInfo {
                i: "23.39",
                p: "23.39^#1:*expr.Constant_FloatValue#",
                e: "",
            },
            TestInfo {
                i: "!a",
...
// )^#2:*expr.Expr_CallExpr#",
p: "",
                e: "ERROR: <input>:1:3: Unknown `in` operator!\n| a in b\n| ..^",
            },
            TestInfo {
                i: "a == b",
                p: "_==_(
...
// | ^",
            ...
            TestInfo {
                i: "1.99e90000009",
                p: "",
                e: "ERROR: <input>:1:1: invalid float literal
| 1.99e90000009
| ^",
            },
            TestInfo {
...
| .........^",
            ...
            TestInfo {
                i: r#"Msg{?field: value} && {?"key": value}"#,
                p: "",
                e: r#"ERROR: <input>:1:5: unsupported syntax '?'
    ...
    impl DebugWriter {
        fn buffer(&mut self, expr: &IdedExpr) -> &Self {
            let e = match &expr.expr {
                ...
                Expr::Literal(val) => match val {
                    ...
                    CelVal::Float(f) => &format!("{f}^#{}:{}#", expr.id, "*expr.Constant_FloatValue"),
                    ...
                    for (i, entry) in map.entries.iter().enumerate() {
                        match &entry.expr {
                            ...
                            EntryExpr::MapEntry(e) => {
                                ...
                                self.push(":");
                                self.buffer(&e.value);
                                self.push(&format!("^#{}:{}#", entry.id, "*expr.Expr_CreateStruct_Entry"));
                            }
                        }
                ...
                Expr::Select(select) => {
                    ...
                    &format!(".{}{}^#{}:{}#", select.field, suffix, expr.id, "*expr.Expr_SelectExpr")
                }
                Expr::Struct(s) => {
        ...
        fn indent(&mut self) {
            if self.line_start {
                ...
                self.buffer
                    .push_str(iter::repeat_n("    ", self.indents).collect::<String>().as_str())
            }
        }
