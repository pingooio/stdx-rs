...
///
/// ```
/// use bel::{Context, Duration, Program};
/// use serde::Serialize;
///
/// #[derive(Serialize)]
/// struct MyStruct {
...
///     .unwrap();
///
/// let program = Program::compile(r#"foo.dur == Duration("2h")"#).unwrap();
...
///
/// ```
/// use bel::{Context, Timestamp, Program};
/// use serde::Serialize;
///
/// #[derive(Serialize)]
/// struct MyStruct {
...
///     .unwrap();
///
/// let program = Program::compile(r#"foo.ts == Timestamp("2025-01-01T00:00:00Z")"#).unwrap();
/// let value = program.execute(&context).unwrap();
/// assert_eq!(value, true.into());
/// ```
#[cfg(feature = "time")]
...
#[cfg(feature = "ip")]
#[derive(Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Debug, Hash)]
pub struct Ip(pub ipnetwork::IpNetwork);

#[cfg(feature = "ip")]
impl Ip {
    // Since serde can't natively represent ips, we serialize a special
    // newtype to indicate we want to rebuild the ip in the result,
    // while remaining compatible with most other Serializer implementations.
    const NAME: &str = "$__cel_private_Ip";
}

#[cfg(feature = "ip")]
impl From<Ip> for ipnetwork::IpNetwork {
...
#[cfg(feature = "ip")]
impl From<ipnetwork::IpNetwork> for Ip {
...
#[cfg(feature = "ip")]
impl From<std::net::IpAddr> for Ip {
    fn from(value: std::net::IpAddr) -> Self {
        Self(ipnetwork::IpNetwork::from(value))
    }
}

#[cfg(feature = "ip")]
impl ser::Serialize for Ip {
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: ser::Serializer,
    {
...
impl ser::Serializer for Serializer {
    ...
    // if the serializer is human_readable, then IpAddr is directly converted to str instead of serialize_newtype_variant
    ...
    fn serialize_u64(self, _v: u64) -> Result<Value> {
        ...
        Err(SerializationError::SerdeError(
            "unsigned intergers are not supported".to_owned(),
        ))
    }
        ...
        T: ?Sized + Serialize,
    {
        match name {
            ...
            #[cfg(feature = "ip")]
            Ip::NAME => value.serialize(IpSerializer {}),
...
impl ser::SerializeStructVariant for SerializeStructVariant {
    ...
    where
        T: ?Sized + Serialize,
    {
        self.map.insert(key.serialize(KeySerializer)?, to_value(value)?);
        Ok(())
    }
...
impl ser::SerializeStruct for SerializeTimestamp {
        ...
        T: ?Sized + Serialize,
    {
        match key {
            ...
            _ => Err(SerializationError::SerdeError("invalid field in duration struct".to_owned())),
        }
    }
...
impl ser::Serializer for KeySerializer {
    ...
    fn serialize_u64(self, _v: u64) -> Result<Key> {
        // Ok(Key::Uint(v))
        Err(SerializationError::SerdeError(
            "unsigned intergers are not supported".to_owned(),
        ))
    ...
    fn serialize_bytes(self, _v: &[u8]) -> Result<Key> {
        Err(SerializationError::InvalidKey("Bytes are not supported".to_string()))
    }

    fn serialize_none(self) -> Result<Key> {
        Err(SerializationError::InvalidKey("None is not supported".to_string()))
    }
    ...
    fn serialize_unit(self) -> Result<Key> {
        Err(SerializationError::InvalidKey("Null is not supported".to_string()))
    }
    ...
    fn serialize_unit_variant(self, _name: &'static str, _variant_index: u32, variant: &'static str) -> Result<Key> {
    ...
    where
        T: ?Sized + Serialize,
    {
        print!("key: serialize_newtype_struct: {_name}");
    ...
    where
        T: ?Sized + Serialize,
    {
        Err(SerializationError::InvalidKey("Newtype variant is not supported".to_string()))
    }

    fn serialize_seq(self, _len: Option<usize>) -> Result<Self::SerializeSeq> {
        Err(SerializationError::InvalidKey("Sequences are not supported".to_string()))
    }

    fn serialize_tuple(self, _len: usize) -> Result<Self::SerializeTuple> {
        Err(SerializationError::InvalidKey("Tuples are not supported".to_string()))
    }

    fn serialize_tuple_struct(self, _name: &'static str, _len: usize) -> Result<Self::SerializeTupleStruct> {
        Err(SerializationError::InvalidKey("Structs are not supported".to_string()))
    }
    ...
    fn serialize_tuple_variant(
        ...
        Err(SerializationError::InvalidKey("Tuple variants are not supported".to_string()))
    }

    fn serialize_map(self, _len: Option<usize>) -> Result<Self::SerializeMap> {
        Err(SerializationError::InvalidKey("Map variants are not supported".to_string()))
    }
    ...
    fn serialize_struct_variant(
        ...
        Err(SerializationError::InvalidKey("Struct variants are not supported".to_string()))
    }
}
...
impl ser::Serializer for TimeSerializer {
    ...
    fn serialize_unit_variant(self, _name: &'static str, _variant_index: u32, _variant: &'static str) -> Result<Value> {
        unreachable!()
    }
...
#[cfg(feature = "ip")]
impl ser::Serializer for IpSerializer {
    type Ok = Value;
    type Error = SerializationError;
    ...
    fn serialize_str(self, v: &str) -> Result<Value> {
        Ok(v.parse::<ipnetwork::IpNetwork>()
            .map_err(|e| SerializationError::SerdeError(e.to_string()))?
            .into())
    }
...
mod tests {
    ...
    use crate::{Context, Program, Value, objects::Key, to_value};
    ...
    fn test_json_data_conversion() {
        ...
        let expected: Value = HashMap::from_iter([
            ...
            (Key::String(Arc::new("f32".to_string())), Value::Float(f64::from(0.32_f32))),
    ...
    fn test_newtype() {
        ...
        let expected: Value = HashMap::from([("Newtype", Value::Int(32))]).into();
        let program = Program::compile("test == expected").unwrap();
        let mut context = Context::default();
        context.add_variable("expected", expected).unwrap();
    ...
    fn test_options() {
        ...
        let expected: Value = HashMap::from([("Wrapped", Value::Int(8))]).into();
        let program = Program::compile("test == expected").unwrap();
        let mut context = Context::default();
        context.add_variable("expected", expected).unwrap();
        context.add_variable("test", wrapped).unwrap();
    ...
    fn test_tuples() {
        ...
        let expected: Value =
            HashMap::from([("Tuple", Value::List(Arc::new(vec![12_i64.into(), 16_i64.into()])))]).into();
        let program = Program::compile("test == expected").unwrap();
        let mut context = Context::default();
        context.add_variable("expected", expected).unwrap();
    ...
    fn test_structs() {
        ...
        let test_struct = TestCompoundTypes::Struct {
            ...
            nested: HashMap::from_iter([(
                true,
                HashMap::from_iter([("Test".to_string(), vec!["a".to_string(), "b".to_string(), "c".to_string()])]),
            )]),
        };
    ...
    fn test_maps() {
        ...
        let map = to_value(TestCompoundTypes::Map(HashMap::<String, &'static Bytes>::from_iter([(
            "Test".to_string(),
        ...
        let expected: Value = HashMap::from([(
            "Map",
            HashMap::<Key, Value>::from_iter([("Test".into(), Value::Bytes(Arc::new(vec![0_u8, 0_u8, 0_u8, 0_u8])))]),
        )])
        .into();
    ...
    fn test_ip_types() {
        ...
        let tests = to_value([IpType {
            ip_addr: IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)).into(),
            ip: ipnetwork::IpNetwork::from(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 2))).into(),
        }])
        .unwrap();
        let expected: Value = vec![Value::Map(
            HashMap::<_, Value>::from([
