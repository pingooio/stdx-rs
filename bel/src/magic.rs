...
use crate::{
    ExecutionError, Expression, FunctionContext, ResolveResult, Value,
    common::ast::Expr,
    macros::{impl_conversions, impl_handler},
    resolvers::{AllArguments, Argument},
};
...
impl From<std::net::IpAddr> for Value {
    fn from(value: std::net::IpAddr) -> Self {
        Value::Ip(ipnetwork::IpNetwork::from(value))
    }
}
...
/// ```
/// # use std::sync::Arc;
/// # use bel::{Program, Context};
/// use bel::extractors::This;
/// # let mut context = Context::default();
/// # context.add_function("starts_with", starts_with);
///
/// /// Notice how `This` refers to the target value when called as a method,
/// /// but the first argument when called as a function.
/// let program1 = r#""foobar".starts_with("foo") == true"#;
/// let program2 = r#"starts_with("foobar", "foo") == true"#;
...
/// In the previous example, the method `starts_with` is only ever called
    ...
    T: FromValue,
{
        ...
        Self: Sized,
    {
        if let Some(ref this) = ctx.this {
            ...
            let arg = arg_value_from_context(ctx).map_err(|_| ExecutionError::missing_argument_or_target())?;
...
impl FunctionRegistry {
    ...
    where
        ...
        T: 'static,
    {
        self.functions.insert(name.to_string(), function.into_function());
    }
