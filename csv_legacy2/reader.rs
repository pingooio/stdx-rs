...
impl<R> Reader<R> {
    ...
    ///
    /// [`parse_headers`]: Reader::parse_headers
    /// [`set_headers`]: Reader::set_headers
...
// ── no_std parsing (from_bytes only, buffer is pre-filled) ─────────────
...
// ── std: streaming from std::io::Read ─────────────────────────────────
...
#[cfg(not(feature = "std"))]
impl<'r, R> Iterator for Rows<'r, R> {
    type Item = Row<'r>;
