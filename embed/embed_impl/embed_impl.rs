...
fn embedded(
    ...
    for embed_utils::FileEntry {
        rel_path,
        full_canonical_path,
    } in embed_utils::get_files(absolute_folder_path.clone(), matcher)
    {
        match_values.insert(
            rel_path.clone(),
            embed_file(
...
fn embed_file(
    ...
    let embedding_code = if metadata_only {
        ...
        let folder_path = folder_path.ok_or(syn::Error::new(
            ident.span(),
            "`folder` must be provided under `compression` feature.",
        ))?;
