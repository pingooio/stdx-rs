    ...
    fn write_final_leftovers(&mut self) -> Result<()> {
        ...
        if self.extra_input_occupied_len > 0 {
            let encoded_len = self
                .engine
                .encode_slice(&self.extra_input[..self.extra_input_occupied_len], &mut self.output[..])
...
impl<'e, E: Engine, W: io::Write> io::Write for EncoderWriter<'e, E, W> {
    ...
    fn write(&mut self, input: &[u8]) -> Result<usize> {
        ...
        if self.extra_input_occupied_len > 0 {
            ...
            if input.len() + self.extra_input_occupied_len >= MIN_ENCODE_CHUNK_SIZE {
                ...
                let len = self
                    .engine
                    .internal_encode(&self.extra_input[0..MIN_ENCODE_CHUNK_SIZE], &mut self.output[..]);
        ...
        encoded_size += self
            .engine
            .internal_encode(&input[..(input_chunks_to_encode_len)], &mut self.output[encoded_size..]);
    ...
    fn flush(&mut self) -> Result<()> {
        self.write_all_encoded_output()?;
        self.delegate.as_mut().expect("Writer must be present").flush()
    }
}
