...
mod tests {
    ...
    #[test]
    fn percent_decode() {
        assert_eq!(super::percent_decode(b"foo%20bar%3f").decode_utf8().unwrap(), "foo bar?");
    }

    #[test]
    fn percent_decode_str() {
        assert_eq!(super::percent_decode_str("foo%20bar%3f").decode_utf8().unwrap(), "foo bar?");
    }
    ...
    fn percent_decode_invalid_utf8() {
        ...
        let decoded = super::percent_decode(b"%00%9F%92%96").decode_utf8().unwrap_err();
    ...
    fn percent_decode_utf8_lossy() {
        assert_eq!(super::percent_decode(b"%F0%9F%92%96").decode_utf8_lossy(), "💖");
    }
