...
impl Buf {
    ...
    fn as_str(&self) -> &str {
        unsafe { str::from_utf8_unchecked(slice::from_raw_parts(self.bytes.as_ptr().cast::<u8>(), self.written)) }
    }
}
...
impl Write for Buf {
    fn write_str(&mut self, s: &str) -> fmt::Result {
        ...
        unsafe {
            ptr::copy_nonoverlapping(s.as_ptr(), self.bytes.as_mut_ptr().add(self.written).cast::<u8>(), s.len());
        }
        self.written += s.len();
        Ok(())
    }
